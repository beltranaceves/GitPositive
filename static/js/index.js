...
(function () {
    var on = addEventListener,
        ...
        client = (function () {
            ...
            if (o.os == 'mac' && ('ontouchstart' in window) && ((screen.width == 1024 && screen.height == 1366) || (
                screen.width == 834 && screen.height == 1112) || (screen.width == 810 && screen.height == 1080) || (
                    screen.width == 768 && screen.height == 1024))) o.os = 'ios';
        ...
        scrollToElement = function (e, style, duration) {
            ...
            if (!e) y = 0;
                ...
                offset = (e.dataset.scrollOffset ? parseInt(e.dataset.scrollOffset) : 0) * parseFloat(getComputedStyle(
                ...
                switch (e.dataset.scrollBehavior ? e.dataset.scrollBehavior : 'default') {
                    ...
                    default:
                        ...
                        break;
                    case 'center':
                        if (e.offsetHeight < window.innerHeight) y = e.offsetTop - ((window.innerHeight - e.offsetHeight) /
                            2) + offset;
                        else y = e.offsetTop - offset;
                        break;
                    case 'previous':
                        if (e.previousElementSibling) y = e.previousElementSibling.offsetTop + e.previousElementSibling
                            .offsetHeight + offset;
                        else y = e.offsetTop + offset;
                        break;
                }
        ...
        (function () {
            if (client.flags.lsdUnits) {
                ...
                var f = function () {
                    ...
                    document.documentElement.style.setProperty('--background-height', (window.innerHeight + 250) +
                        'px');
                };
                on('load', f);
                on('resize', f);
    ...
    if (client.os == 'android') {
        ...
        elements.forEach(function (e) {
            var desktop = [],
                ...
                state = false,
                query, a, x, ce, f;
            if ('reorderBreakpoint' in e.dataset && e.dataset.reorderBreakpoint in breakpoints) query =
                breakpoints[e.dataset.reorderBreakpoint];
                ...
                if (item.initialState !== null) {
                    ...
                    switch (item.mode) {
                        ...
                        case 3:
                            ...
                            b = top + (height * 0.75);
                            if (b + (height * 0.375) >= document.body.scrollHeight - scrollPad) b = document.body
                                .scrollHeight + scrollPad;
        ...
        effects: {
            'blur-in': {
                transition: function (speed, delay) {
                    return 'opacity ' + speed + 's ease' + (delay ? ' ' + delay + 's' : '') + ', ' + 'filter ' + speed +
                        's ease' + (delay ? ' ' + delay + 's' : '');
                },
            ...
            'zoom-in': {
                transition: function (speed, delay) {
                    return 'opacity ' + speed + 's ease' + (delay ? ' ' + delay + 's' : '') + ', ' + 'transform ' +
                        speed + 's ease' + (delay ? ' ' + delay + 's' : '');
                },
            ...
            'focus-image': {
                target: 'img',
                transition: function (speed, delay) {
                    return 'transform ' + speed + 's ease' + (delay ? ' ' + delay + 's' : '') + ', ' + 'filter ' +
                        speed + 's ease' + (delay ? ' ' + delay + 's' : '');
                },
                rewind: function (intensity) {
            ...
            $$(selector).forEach(function (e) {
                ...
                if (e.parentNode) {
                    if (e.parentNode.dataset.onvisibleTrigger) triggerElement = e.parentNode;
                        ...
                        if (e.parentNode.parentNode.dataset.onvisibleTrigger) triggerElement = e.parentNode
                            .parentNode;
                    }
                }
                scrollEvents.add({
